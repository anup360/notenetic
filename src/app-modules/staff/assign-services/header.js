...
const AssignServiceHeader = () => {
  ...
  const AvailableServices = () => {
    ...
    apiHelper.getRequest(
      ...
      .then((result) => {
        ...
        if ( userAccessPermission[permissionEnum.MANAGE_STAFF_SERVICES]) {
  ...
  const AssignedServices = () => {
    setLoading(true);
    var id = Encrption(selectedStaffId);
    apiHelper.getRequest(API_URLS.GET_STAFF_ASSIGNED_SERVICES + id + "&active=" + 1)
      .then((result) => {
        const data = result.resultData;
        setAssigned(data);
        setLoading(false);
