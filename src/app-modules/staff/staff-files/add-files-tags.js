...
const AddDocumentTags = ({ onClose, docId, staffTags }) => {
  ...
  const getStaffTags = async () => {
    await SettingsService.getClinicTags(clinicId)
      .then((result) => {
        ...
        let newArray =list.filter((element)=>!staffTags.includes(element.color))
  ...
  const assignTags = async () => {
    setLoading(true);
    await StaffService.assignTagToStaffDocument(fields, docId)
      .then((result) => {
        setLoading(false);
